...
static inline uint32_t rol(uint32_t v, uint32_t b) {
	return (v << b) | (v >> (32 - b));
}
...
static JSValue js_hash_sha1sum(JSContext *ctx, JSValueConst this_val,
								int argc, JSValueConst *argv)
{
	return js_digest( ctx, this_val, argc, argv, DIGEST_SHA1 );
}
...
static const JSCFunctionListEntry js_hash_funcs[] = {
	...
	JS_CFUNC_DEF("sha1sum", 1, js_hash_sha1sum ),
